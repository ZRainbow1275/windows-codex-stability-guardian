...
| [openai/codex#17540](https://github.com/openai/codex/issues/17540) | Windows 桌面应用重启后侧边栏历史会话丢失，但磁盘上 sessions 还在（open） | **C2** | 备份 `state_*.sqlite` → 清除最新 state DB 中的 stale rows（`threads.has_user_event` 漂移），让 Codex 重新认得磁盘上的会话；传入 `--project-path` 时只修该 cwd |
...
- 在 `--confirm` 下：
  - 备份 SQLite → 清理 stale rows → post-write 校验；如果传入 `--project-path`，只修该项目 cwd 的 `threads.has_user_event` 漂移
...
guardian.exe --json repair codex --dry-run
guardian.exe --json repair codex --confirm      # 全局备份 sqlite → 清 stale rows → 校验

# 如果只是某一个项目的 /resume 看不到历史，优先限定项目路径：
guardian.exe --json repair codex --dry-run --project-path "D:\path\to\project"
guardian.exe --json repair codex --confirm --project-path "D:\path\to\project"
```

修完之后重启 Codex 桌面应用，侧边栏历史应该重新出现。`--project-path` 模式不会归档、移动或删除 session，只会在备份后修复该 cwd 下非空 `first_user_message` 线程的 `has_user_event` 漂移，并同步安装 resume picker wrapper。
