...
fn localized_codex_summary(summary: &str) -> String {
        ...
        )
    {
        ...
        return format!(
            "Codex 确认修复已执行：{translated}，但仍有 30 天以前的 sessions 未完成归档。"
        );
    }
    if let Some(details) = summary.strip_prefix("Codex repair confirm ")
        && let Some(details) =
...
mod tests {
    ...
    fn translates_managed_codex_trust_notes() {
        let note = localized_report_note(
            "Codex project trust is missing for `D:\\Workspaces\\Inkforge`; Guardian found 2 expected lookup key(s) absent from `config.toml`.",
        );
        assert!(note.contains("D:\\Workspaces\\Inkforge"));
