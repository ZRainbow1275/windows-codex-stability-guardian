...
fn localized_evidence_label(domain: &str, key: &str) -> String {
    let label = match (domain, key) {
        ...
        ("codex", "codex_config_path") => Some("Codex 配置路径"),
        ("codex", "codex_config_present") => Some("Codex 配置已存在"),
        ("codex", "trust_target_path") => Some("trust 目标项目"),
        ("codex", "trust_target_source") => Some("trust 目标来源"),
        ("codex", "trust_missing_lookup_keys") => Some("缺失 trust lookup keys"),
        ("codex", "repair_outcome") => Some("修复结果"),
        ("codex", "repair_state_db") => Some("修复目标 state 数据库"),
        ("codex", "repair_stale_before") => Some("修复前 stale rows"),
        ("codex", "repair_stale_after") => Some("修复后 stale rows"),
        ("codex", "repair_audit_path") => Some("修复审计路径"),
        ("codex", "repair_backup_path") => Some("state 备份路径"),
        ("codex", "repair_trust_target_path") => Some("修复 trust 目标项目"),
        ("codex", "repair_trust_target_source") => Some("修复 trust 目标来源"),
        ("codex", "repair_trust_config_path") => Some("修复写入的 Codex 配置"),
        ("codex", "repair_trust_created_config") => Some("修复时新建 Codex 配置"),
        ("codex", "repair_trust_config_backup_path") => Some("Codex 配置备份路径"),
        ("codex", "repair_trust_missing_keys_before") => Some("修复前缺失 trust keys"),
        ("codex", "repair_trust_added_keys") => Some("本次补写 trust keys"),
        ("codex", "repair_slow_path_launcher_path") => Some("slow-path launcher 目标"),
        ("codex", "repair_slow_path_launcher_backup_path") => Some("launcher 备份路径"),
        ("codex", "repair_slow_path_helper_path") => Some("resume picker helper 路径"),
        ("codex", "repair_slow_path_metadata_path") => Some("resume picker metadata 路径"),
        ("codex", "repair_slow_path_launcher_updated") => Some("launcher 已更新"),
        ("codex", "repair_slow_path_helper_updated") => Some("resume picker helper 已更新"),
...
fn localized_evidence_value(value: &str) -> String {
    match value {
        ...
        "requested_project_path" => "命令显式提供的项目路径".to_string(),
        "codex_tui_warning" => "Codex TUI trust 警告".to_string(),
        "noop" => "无需修复".to_string(),
        "repaired" => "已修复".to_string(),
        "unresolved" => "未完全修复".to_string(),
