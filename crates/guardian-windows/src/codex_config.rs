...
mod tests {
    ...
    fn expected_keys_include_plain_and_unc_lowercase_variants() {
        let keys = expected_project_trust_keys(Path::new(r"D:\Workspaces\Inkforge"));
        assert_eq!(
            keys,
            vec![
                r"d:\workspaces\inkforge".to_string(),
                r"\\?\d:\workspaces\inkforge".to_string(),
            ]
        );
    }
    ...
    fn parser_only_collects_trusted_project_entries() {
        ...
        assert!(keys.contains(r"d:\workspaces\inkforge"));
        assert!(!keys.contains(r"d:\workspaces\other"));
    }
    ...
    fn missing_keys_ignore_existing_trusted_entries() {
        ...
        assert_eq!(missing, vec![r"\\?\d:\workspaces\inkforge".to_string()]);
    }
